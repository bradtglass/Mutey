namespace Mutey.Hue.ViewModels.Setup
{
    ...
    public class LightSetupViewModel : ObservableObject
    {
        ...
        {
            var parts = Fields.OfType<IAffectsColor>()
                              .Select( f => f.GetColorComponents() )
                              .ToList();

            byte a = parts.Select( p => p.a ).FirstOrDefault( a => a.HasValue ) ?? byte.MaxValue;
            ( byte r, byte g, byte b ) = parts.Select( p => p.Item2 ).FirstOrDefault( rgb => rgb.HasValue ) ?? ( 255, 229, 207 );
        ...
        {
            bool? on = Fields.OfType<LightOnSetupViewModel>().FirstOrDefault()?.On;
            byte? brightness = Fields.OfType<LightBrightnessSetupViewModel>().FirstOrDefault()?.GetBrightness();
