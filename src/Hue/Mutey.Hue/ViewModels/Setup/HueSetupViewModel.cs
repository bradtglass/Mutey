namespace Mutey.Hue.ViewModels.Setup
{
    ...
    public class HueSetupViewModel : ObservableObject
    {
        ...
        private async Task LoadLights()
        {
            ...
            var lights = await Task.Run( context.GetLightsAsync );
        ...
        private void SelectableChanged( object? sender, PropertyChangedEventArgs e )
        {
            ...
            var selectable = (SelectableViewModel<LightInfo>) sender!;
        ...
        {
            for ( int i = viewModel.Lights.Count - 1; i >= 0; i-- )
            {
        ...
        {
            return new HueStateSetting( viewModel.Lights.ToDictionary( s => s.Info.Name, s => s.GetSetting() ) );
        }
    }
}
